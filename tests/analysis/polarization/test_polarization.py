...
def test_polarization(enable_backend, full_graph_edgelist: pd.DataFrame):
    ...
    expected = pd.DataFrame(
        data={
            "morans_i": [-0.057901599737577816, -0.05840524191067242],
            "morans_p_value": [0.0007189388965430386, 0.0006311197019152904],
            "morans_p_adjusted": [0.0007189388965430386, 0.0007189388965430386],
            "morans_z": [-3.382253033381628, -3.417875495725142],
            "marker": ["A", "B"],
...
def test_permuted_polarization(enable_backend, full_graph_edgelist: pd.DataFrame):
    ...
    expected = pd.DataFrame(
        data={
            ...
            "morans_z_sim": {0: -3.230459041258403, 1: -3.973112435543935},
            "marker": ["A", "B"],
            "component": ["PXLCMP0000000", "PXLCMP0000000"],
        },
        index=pd.Index([0, 1]),
