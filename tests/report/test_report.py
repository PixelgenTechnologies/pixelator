    ...
    assert_frame_equal(
        ...
        pd.DataFrame(
            data={
                ...
                "reads_in_aggregates": np.array([18000, 18000], dtype=np.uint64),
                "number_of_aggregates": [3, 3],
                "fraction_of_aggregates": [0.6, 0.6],
                "molecules_in_aggregates": np.array([18000, 18000], dtype=np.int64),
                "molecules_pct_in_aggregates": [0.6, 0.6],
...
def test_create_dynamic_report(tmp_path):
    with mock.patch(
        ...
        create_dynamic_report(
            ...
            summary_collapse=pd.Series({"input": 900, "output_molecules": 200}),
