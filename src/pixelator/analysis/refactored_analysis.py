...
class ColocalizationAnalysisSweep(PerComponentAnalysis):
    ANALYSIS_NAME = "colocalization"
    ...
    def run_on_component(self, component: Graph, component_id: str) -> pd.DataFrame:
        logger.debug(
            "Running colocalization sweep analysis on component %s", component_id
        )

        def _run_parameter_combinations(self, component, component_id):
            for ns in self.neighbourhood_size:
                for tf in self.transformation_type:
                    coloc_df = colocalization_from_component_graph(
                        graph=component,
                        component_id=component_id,
                        transformation=tf,
        ...
        return pd.concatenate(
            (_run_parameter_combinations(self, component, component_id)),
            axis=0,
        )
