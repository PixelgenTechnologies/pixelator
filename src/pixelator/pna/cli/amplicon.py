...
def amplicon(
    ...
    Args:
        ...
        sample_name: Override the basename of the output fastq file. Default is the basename of the
            first input file without extension and read 1 identifier.
        ...
        low_complexity_filter: Enable filtering of amplicons with low complexity UMI sequences.
        low_complexity_threshold: The threshold for determining if a UMI sequence is of low
            complexity and will be removed.
        lbs_filter: Enable filtering of amplicons with UMI sequences that show similarity to the LBS
            sequence.
        lbs_filter_min_overlap: The minimum overlap required in a semi-global alignment of a UMI
            sequence with the LBS regions. Reads with less overlap will not be removed by the LBS
            filter.
        lbs_filter_error_rate: The allowed error-rate in a semi-global alignment of a UMI sequence
            with the LBS regions.The error rate is defined as a percentage of UMI length.
        skip_input_checks: Skip all check on the filename of input fastq files.
        force_run: If more than 50% of reads are filtered, this indicates some serious problem with
            the input data and the amplicon commandwill fail with an exit status of 1. Setting this
            flag will force an exit status of 0.
